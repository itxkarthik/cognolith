...
def list_notes(
	*,
	...
	notes = session.exec(statement.order_by(col(Notes.updated_at).desc())).all()

	if tag_id is not None:
		notes = [note for note in notes if any(tag.id == tag_id for tag in note.tags)]

	total = len(notes)
	paged = notes[skip : skip + limit]
	return paged, total
...
def update_note(*, session: Session, current_user: User, note_id: int, payload: NoteUpdate) -> Notes:
	note = get_note_by_id(session=session, current_user=current_user, note_id=note_id)
	update_data = payload.model_dump(exclude_unset=True)
	if not update_data:
		return note

	_validate_folder_access(session=session, user_id=current_user.id, folder_id=update_data.get("folder_id"))
	_validate_document_access(session=session, user_id=current_user.id, document_id=update_data.get("linked_document_id"))
...
def create_folder(*, session: Session, current_user: User, payload: FolderCreate) -> NoteFolders:
	_validate_folder_access(session=session, user_id=current_user.id, folder_id=payload.parent_folder_id)
	folder = NoteFolders(
		user_id=current_user.id,
		name=payload.name,
...
def move_note_to_folder(*, session: Session, current_user: User, note_id: int, folder_id: int | None) -> Notes:
...
def assign_tags_to_note(*, session: Session, current_user: User, note_id: int, tag_ids: list[int]) -> Notes:
	note = get_note_by_id(session=session, current_user=current_user, note_id=note_id)
	if not tag_ids:
		note.tags = []
		session.add(note)
...
def remove_tag_from_note(*, session: Session, current_user: User, note_id: int, tag_id: int) -> Notes:
	note = get_note_by_id(session=session, current_user=current_user, note_id=note_id)
...
def link_notes(*, session: Session, current_user: User, source_note_id: int, target_note_ids: list[int]) -> None:
	...
	for target_note_id in set(target_note_ids):
		if source_note_id == target_note_id:
			continue
		target_note = get_note_by_id(session=session, current_user=current_user, note_id=target_note_id)
		existing = session.exec(
...
def sync_note_links(*, session: Session, current_user: User, source_note_id: int, target_note_ids: list[int]) -> None:
	source_note = get_note_by_id(session=session, current_user=current_user, note_id=source_note_id)
...
def link_note_to_document(*, session: Session, current_user: User, note_id: int, document_id: int | None) -> Notes:
